...
static lv_obj_t *labels[5], *bars[HISTORY_LEN];
static bool screen;
...
static void flush(lv_display_t *d, const lv_area_t *a, uint8_t *pixels) {
  if (screen) {
    ...
    if (esp_lcd_panel_draw_bitmap(panel, a->x1, a->y1, a->x2 + 1, a->y2 + 1,
                                  dma_pixels) != ESP_OK ||
...
static bool lcd_init(void) {
  ...
  if (!dma_done)
    return false;
  spi_bus_config_t bus = ST7735_PANEL_BUS_SPI_CONFIG(
      BOARD_LCD_CLK, BOARD_LCD_MOSI, BOARD_WIDTH * 16 * 2);
  if (spi_bus_initialize(SPI2_HOST, &bus, SPI_DMA_CH_AUTO) != ESP_OK)
    return false;
  esp_lcd_panel_io_handle_t io;
  esp_lcd_panel_io_spi_config_t ic = ST7735_PANEL_IO_SPI_CONFIG(
      BOARD_LCD_CS, BOARD_LCD_DC, transfer_done, NULL);
  ic.pclk_hz = 20000000;
  ic.trans_queue_depth = 1;
  if (esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)SPI2_HOST, &ic, &io) !=
      ESP_OK)
    return false;
  ...
  if (esp_lcd_panel_invert_color(panel, true) != ESP_OK ||
    ...
    return false;
  dma_pixels = heap_caps_malloc(BOARD_WIDTH * 16 * 2,
...
static void task(void *arg) {
  ...
  gpio_config(&button);
#if CONFIG_ADAPTER_LED
  gpio_config_t led = {.pin_bit_mask =
                           (1ULL << BOARD_LED_DATA) | (1ULL << BOARD_LED_CLK),
  ...
  for (;;) {
    ...
    button_event_t ev =
        button_update(&btn, !gpio_get_level(BOARD_BUTTON), dim, now);
    if (ev != BUTTON_NONE) {
      last_touch = now;
      dim = false;
      if (ev == BUTTON_SHORT) {
    ...
    if (now - last_draw >= 250) {
      ...
      view_t v = {.page = page,
                  ...
                  .uptime = now / 1000,
                  .connection_uptime =
                      b.associated ? (now - b.connected_ms) / 1000 : 0,
                  .drops = b.stats.rxdrop + b.stats.txdrop + b.stats.poolfail +
                           b.stats.malformed,
      ...
      v.ip_valid = b.associated &&
                   address_fresh(b.addresses.valid4, b.addresses.seen4, now);
      if (v.ip_valid) {
        snprintf(v.ip, sizeof(v.ip), "%u.%u.%u.%u", b.addresses.ip4[0],
                 b.addresses.ip4[1], b.addresses.ip4[2], b.addresses.ip4[3]);
        ...
        if (confirm) {
          ...
          if (menu == 0)
            ...
            snprintf(tmp, sizeof(tmp), "%d %s", menu,
                     a.cfg.p[menu - 1].ssid[0] ? a.cfg.p[menu - 1].name
                                               : "(empty)");
      ...
      if (a.setup || a.active < 0)
        ...
        led_color(0, 180, 0);
      else if (!b.associated && (b.last_reason == 201 || b.last_reason == 202 ||
                                 b.last_reason == 15))
        led_color(180, 0, 0);
      else
