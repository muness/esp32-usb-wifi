  ...
  for (;;)
    if (xQueueReceive(output, b, portMAX_DELAY) == pdTRUE) {
      ...
      while (off < n && retries++ < 10) {
        off += tinyusb_cdcacm_write_queue(TINYUSB_CDC_ACM_0,
                                          (const uint8_t *)b + off, n - off);
...
static void state(int itf, cdcacm_event_t *e) {
  if (e->line_state_changed_data.dtr)
    mgmt_write(
        "T-Dongle-S3 adapter 0.1.0. Type help. Input is not echoed.\r\n");
}
void console_init(void) {
  output = xQueueCreate(32, 128);
  assert(output);
  assert(xTaskCreate(writer, "console_tx", 3072, NULL, 2, NULL) == pdPASS);
  tinyusb_config_cdcacm_t c = {.cdc_port = TINYUSB_CDC_ACM_0,
                               .callback_rx = rx,
                               .callback_line_state_changed = state};
