...
static bridge_crash_info_t
    s_crash; /* boot-time evaluation, served to the console */
...
static void crashlog_boot(void) {
  ...
  bool warm = (rr != ESP_RST_POWERON && rr != ESP_RST_BROWNOUT &&
...
bool bridge_host_ipv4(uint8_t ip[4]) {
  portENTER_CRITICAL(&s_lock);
  bool v =
      address_fresh(s_addr.valid4, s_addr.seen4, esp_timer_get_time() / 1000);
  if (v)
...
bool bridge_host_ipv6(uint8_t ip[16]) {
  portENTER_CRITICAL(&s_lock);
  bool v =
      address_fresh(s_addr.valid6, s_addr.seen6, esp_timer_get_time() / 1000);
  if (v)
...
static esp_err_t pkt_wifi2usb(void *buffer, uint16_t len, void *eb) {
  ...
  if (len < 14 || len > FRAME_MAX)
    ...
    COUNT(reflected);
  else if (!tud_ready() || !bridge_wifi_connected() ||
           xQueueReceive(s_free, &i, 0) != pdTRUE)
    COUNT(rxdrop);
  else {
    portENTER_CRITICAL(&s_lock);
...
static void tx_worker(void *arg) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(s_pending, &i, pdMS_TO_TICKS(100)) == pdTRUE) {
      ...
      portEXIT_CRITICAL(&s_lock);
      esp_err_t e =
          current && bridge_wifi_connected() && tud_ready()
              ? tinyusb_net_send_sync(s_frames[i].bytes, len,
                                      (void *)(uintptr_t)i, pdMS_TO_TICKS(20))
...
void app_main(void) {
  ...
  if (e != ESP_OK) {
    ESP_LOGE(TAG, "NVS unavailable: %s; ROM recovery required",
             esp_err_to_name(e));
    return;
  }
  ...
  snprintf(serial, sizeof(serial), "%02X%02X%02X%02X%02X%02X", s_mac[0],
           s_mac[1], s_mac[2], s_mac[3], s_mac[4], s_mac[5]);
  ...
  const esp_timer_create_args_t snap = {.callback = snapshot_timer_cb,
                                        .name = "crashsnap"};
