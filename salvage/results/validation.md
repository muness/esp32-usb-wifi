...
The current readability-restacked host candidate has not been rerun locally; its checked-in host-test logs are explicitly marked as historical. No hardware tests, flashing, TeslaAndroid changes, physical throughput measurements or upstream PRs were performed. These donor binaries are build evidence only, not T-Dongle release artifacts. Integration commit `34b239d` has the exact tree of application `2634731`, verified by an empty Git diff; the preserved application was not rebuilt for this history-only merge.
