...
`tdongle/integration` preserves the original application tree from `2634731ff64b1a96683da4c362960f6bde5cc024` and its Git history, joined to the formal fork with an explicit merge. It is separate from the candidate stack; it is not represented as a composition of those commits. Original source remains in the sibling `t-dongle-s3-adapter` checkout. Its latest firmware-code commit is `61eda53`; the later commit changes documentation.
