...
| `stack/02-observation-lifecycle` | `b428065` | stack/01 | Expire observations after 60 seconds; clear on Wi-Fi/USB/network transitions; reject stale in-flight observations using epochs |
| `stack/03-coherent-byte-counters` | `5d8f631` | stack/02 | Locked snapshots of frame/drop/64-bit byte counters and console output; concurrent host tests |

The parser names the Ethernet/IP/ARP field offsets and protocol values, and comments explain the length and version checks; its tests use named frame fields and readable fixture helpers. The branch stack has been restacked after the readability passes. The earlier host-test logs are historical; the candidate tests have not been rerun after these formatting/API-name changes. CI on `tdongle/integration` builds the separate T-Dongle product snapshot and does not test these donor branch files. The 60-second observation lifetime is an explicit policy choice for maintainer review. The parser preserves the donor's global IPv6 2000::/3 scope; it does not claim new link-local/multicast forwarding capabilities. Static hosts can populate observations without DHCP. Byte counters are per boot; the existing warm-reset RTC record format remains unchanged. Counters measure successful send acceptance, not confirmed delivery at the remote endpoint.
