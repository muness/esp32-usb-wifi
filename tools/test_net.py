...
subprocess.run(shlex.split(os.environ.get('CC', 'cc')) + shlex.split(os.environ.get('TEST_CFLAGS', '')) + ['-std=c11', '-Wno-pragma-once-outside-header', 'build-host/test_net.c', '-o', 'build-host/test_net'], check=True)
